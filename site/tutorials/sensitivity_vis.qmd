...
plot_sensitivity <- function(data, x_var, color_var, zoom = FALSE, add_rect = FALSE, title = "") {
    ...
    plot_data <- data |>
        ...
        mutate(label = recode(name, hs0 = "sqrt(lambda[1])", hs1 = "sqrt(lambda[2])"))
    ...
    if (add_rect) {
        p <- p +
            geom_rect(
                data = tibble(
                    label = c("sqrt(lambda[1])", "sqrt(lambda[2])"),
                    xmin = -Inf,
                    xmax = Inf,
                    ymin = c(0, 0),
                    ymax = c(1, 0.05)
...
p1 <- plot_sensitivity(sensitivity, x_var = "scaling_epps", color_var = "radius",
                      add_rect = TRUE, title = "A")
